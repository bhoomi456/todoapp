...
// Get the todos and initialize the list
...
function saveTodos() {
    try {
        const todosJson = JSON.stringify(allTodos);  // Convert the array to JSON string
...
function getTodos() {
    ...
    if (todos) {
        try {
            return JSON.parse(todos);  // Try to parse the JSON string
        } catch (error) {
    ...
    return [];  // Return an empty array if nothing is stored in localStorage
}
